...
async function main() {
  ...
  const unip = new Token(42161, "0xfc5A1A6EB076a2C7aD06eD22C90d7E710E35ad0a", 18, "SYMBOL", "NAME")
  ...
  console.log("NFT ID,Start (ETH),End (ETH),ETH Liquidity, UNIP Liquidity")
  for (let i = 0; i < nftIds.length; i++) {
    ...
    const start = tickToPrice(unip, weth, positionInfo.tickUpper).toSignificant(6)
    const end = tickToPrice(unip, weth, positionInfo.tickLower).toSignificant(6)
    const ethLiquidity = position.amount0.toSignificant(6)
    const unipLiquidity = position.amount1.toSignificant(6)

    console.log(`${nftId},${start},${end},${ethLiquidity},${unipLiquidity}`)
  }
}
