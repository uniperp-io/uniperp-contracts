...
const stakeUnipList = require("../../data/stakeUnipList.json")
...
async function main() {
  ...
  for (let i = 0; i < stakeUnipList.length; i++) {
    const { address, balance } = stakeUnipList[i]

    const stakedAmount = await stakedUnipTracker.stakedAmounts(address)
    ...
    const pendingRewards = await stakedUnipTracker.claimable(address)
    const pendingBonus = await bonusUnipTracker.claimable(address)
