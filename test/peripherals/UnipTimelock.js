...
describe("UnipTimelock", function () {
  ...
  beforeEach(async () => {
    ...
    timelock = await deployContract("UnipTimelock", [
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
  ...
  it("inits", async () => {
    ...
    await expect(deployContract("UnipTimelock", [
      wallet.address,
      7 * 24 * 60 * 60 + 1,
      7 * 24 + 60 * 60,
      rewardManager.address,
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("UnipTimelock: invalid _buffer")
    ...
    await expect(deployContract("UnipTimelock", [
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("UnipTimelock: invalid _longBuffer")
  })
  ...
  it("setTokenConfig", async () => {
    ...
    await expect(timelock.connect(wallet).setTokenConfig(
      ...
      0,
      0
    )).to.be.revertedWith("UnipTimelock: token not yet whitelisted")
  ...
  it("setBuffer", async () => {
    const timelock0 = await deployContract("UnipTimelock", [
      user1.address,
      3 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      rewardManager.address,
    ...
    await expect(timelock0.connect(user1).setBuffer(3 * 24 * 60 * 60 - 10))
      .to.be.revertedWith("UnipTimelock: buffer cannot be decreased")
  ...
  it("approve", async () => {
    ...
    await expect(timelock.connect(wallet).cancelAction(action0))
      .to.be.revertedWith("UnipTimelock: invalid _action")
  ...
  it("addExcludedToken", async () => {
    const unip = await deployContract("UNIP", [])
    await expect(timelock.connect(user0).addExcludedToken(unip.address))
      .to.be.revertedWith("UnipTimelock: forbidden")

    expect(await timelock.excludedTokens(unip.address)).eq(false)
    await timelock.connect(wallet).addExcludedToken(unip.address)
    expect(await timelock.excludedTokens(unip.address)).eq(true)
  })
  ...
  it("setInPrivateTransferMode", async () => {
    ...
    await timelock.addExcludedToken(unip.address)
    await expect(timelock.connect(wallet).setInPrivateTransferMode(unip.address, true))
      .to.be.revertedWith("UnipTimelock: invalid _inPrivateTransferMode")
  ...
  it("setExternalAdmin", async () => {
    ...
    await expect(timelock.connect(wallet).setExternalAdmin(timelock.address, user3.address))
      .to.be.revertedWith("UnipTimelock: invalid _target")
  })
