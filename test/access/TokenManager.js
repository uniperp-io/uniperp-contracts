...
describe("TokenManager", function () {
  ...
  beforeEach(async () => {
    ...
    unipTimelock = await deployContract("UnipTimelock", [
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      user0.address,
  ...
  it("approve", async () => {
    ...
    await expect(tokenManager.connect(wallet).approve(unip.address, user2.address, expandDecimals(5, 18), 1))
      .to.be.revertedWith("TokenManager: action not signalled")
  ...
  it("signalSetGov", async () => {
    await expect(tokenManager.connect(user0).signalSetGov(timelock.address, unip.address, user1.address))
      .to.be.revertedWith("TokenManager: forbidden")

    await tokenManager.connect(wallet).signalSetGov(timelock.address, unip.address, user1.address)
  })

  it("signSetGov", async () => {
    await expect(tokenManager.connect(user0).signSetGov(timelock.address, unip.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: forbidden")

    await expect(tokenManager.connect(signer2).signSetGov(timelock.address, unip.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")
    ...
    await tokenManager.connect(signer2).signSetGov(timelock.address, unip.address, user1.address, 1)

    await expect(tokenManager.connect(signer2).signSetGov(timelock.address, unip.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: already signed")

    await tokenManager.connect(signer1).signSetGov(timelock.address, unip.address, user1.address, 1)
  })

  it("setGov", async () => {
    await unip.setGov(unipTimelock.address)

    await expect(tokenManager.connect(user0).setGov(unipTimelock.address, unip.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: forbidden")

    await expect(tokenManager.connect(wallet).setGov(unipTimelock.address, unip.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await tokenManager.connect(wallet).signalSetGov(unipTimelock.address, unip.address, user1.address)

    await expect(tokenManager.connect(wallet).setGov(user2.address, unip.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(unipTimelock.address, user0.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(unipTimelock.address, unip.address, user2.address, 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(unipTimelock.address, unip.address, user1.address, 1 + 1))
      .to.be.revertedWith("TokenManager: action not signalled")

    await expect(tokenManager.connect(wallet).setGov(unipTimelock.address, unip.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: action not authorized")

    await tokenManager.connect(signer0).signSetGov(unipTimelock.address, unip.address, user1.address, 1)

    await expect(tokenManager.connect(wallet).setGov(unipTimelock.address, unip.address, user1.address, 1))
      .to.be.revertedWith("TokenManager: insufficient authorization")

    await expect(unipTimelock.connect(wallet).signalSetGov(unip.address, user1.address))
      .to.be.revertedWith("UnipTimelock: forbidden")

    await tokenManager.connect(signer2).signSetGov(unipTimelock.address, unip.address, user1.address, 1)

    await expect(unipTimelock.connect(wallet).setGov(unip.address, user1.address))
      .to.be.revertedWith("UnipTimelock: action not signalled")

    await tokenManager.connect(wallet).setGov(unipTimelock.address, unip.address, user1.address, 1)
    ...
    expect(await unip.gov()).eq(unipTimelock.address)
    await unipTimelock.connect(wallet).setGov(unip.address, user1.address)
    expect(await unip.gov()).eq(user1.address)
  })
})
