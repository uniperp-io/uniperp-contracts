...
contract EsUnipBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esUnip;

    constructor(address _esUnip) public {
        admin = msg.sender;
        esUnip = _esUnip;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsUnipBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esUnip).transferFrom(msg.sender, _accounts[i], _amounts[i]);
